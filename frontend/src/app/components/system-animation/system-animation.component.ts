...
export class SystemAnimationComponent implements OnInit, OnChanges {
  ...
  //Setting camera position (Maybe change camera position using scales ?)
  ...
  private setupPostProcessing(): void {
    ...
    this.composer.addPass(renderScene); //render the scene
    this.composer.addPass(bloomPass); //then apply bloom
  }
  ...
  private startFollowing(): void {
    ...
    const targetObject = this.clickableObjects.find(mesh =>
      mesh.userData['type'] === this.followedObject.type &&
      mesh.userData['data'][`${this.followedObject.type}_id`] === this.followedObject.id
    );

    if (targetObject) {
      const distance = this.calculateViewDistance(targetObject);
      const offset = new THREE.Vector3(0, distance * 0.3, distance);
      this.camera.position.copy(targetObject.position.clone().add(offset));
      this.controls.enabled = false;
    }
  }
